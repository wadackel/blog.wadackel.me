...
公開されているReactコンポーネントのテストコードを見てみると、[enzyme](https://github.com/airbnb/enzyme)を使ってテストしているものを結構見かけます。enzymeは[React公式](https://facebook.github.io/react/docs/test-utils.html)でも押しているっぽいので、積極的に使っていきたいです。
...
> [ReactのテストをEnzymeで書いてみよう - Qiita](http://qiita.com/syossan27/items/4a66b4fe5d6c19a4df84)
...
* マウント後、3秒後に`onRequestClose`コールバックを呼び出す
...
あと、`onRequestClose`が呼ばれたかどうか、3秒後に◯◯する、などの処理を簡単にテストしたいので`sinon`もインストールしておきます。これは表題に直接関係無いのでお好みで。
...
`test`ディレクトリを作成後、jsdomの設定ファイルとテストファイルを追加します。
...
* 後述するjsdomの設定ファイル`.setup.js`を読み込み
...
`componentDidMount`内で設定した`setTimeout`が正常に動き、マウント後3秒経過時に`onRequestClose`が呼び出されていることが確認できました。

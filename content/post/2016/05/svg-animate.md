...
しかし、既に廃止予定のAPIで**非推奨**となっています。ChromeではSIMLを使ったSVGを表示すると、コンソールに以下の様に警告が表示されるので今後使う機会は減っていくと思います。
...
SMILでは、2つのイラスト(`d`属性値)を丸々変える方法でしたが、CSSでは動かす部分のみ分割し、分割したパーツに対してCSSTransiion、又はCSSAnimationを適用していきます。  
...
2枚絵としていた部分を、動かす部分とそうで無い部分で分割します。

![パーツの分割図]({{% image "css_parts.png" %}})

分割したものをSketch上でグループ化しておき、SVGとして書き出しておきます。あとは、<a href="#toc_2" data-scroll>SVGの書き出し</a>と同じ要領でコードを最適化しておきます。
...
胴体としっぽのパスを`g`でグルーピングして、それぞれの`path`にはCSSから操作しやすいようにクラスを付与しています。

あとはCSSでしっぽの部分をフリフリさせれば完成です。
...
* 単純な動きであれば、CSSで充分
* 複雑なパスアニメーションが必要になった場合は専用のライブラリに頼る方が良さそう
